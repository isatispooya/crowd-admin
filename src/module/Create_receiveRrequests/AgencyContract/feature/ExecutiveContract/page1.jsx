          ...
          {guarantor
            ...
            .map((item, index) => (
              <p key={`physical-guarantor-${index}`}>
                ...
                {item?.members?.guarantor_national_id} و شماره تماس {item?.members?.phone_number} متولد{' '}
                {item?.members?.birth_date} به آدرس {item?.members?.guarantor_address} واحد{' '}
                {item?.members?.unit} به کد پستی {item?.members?.postal_code} که از این پس در این قرارداد به عنوان «ضامن حقیقی» معرفی می‌گردد.
              </p>
            ))}
          {guarantor
            .filter((g) => g.guarantor_national_id !== 'physical')
            .map((item, index) => (
              <p key={`legal-guarantor-${index}`}>
                {index + 3 + guarantor.filter((g) => g.guarantor_national_id === 'physical').length}) شرکت {item.company_agent} ({item.kind_of_company}) به شناسه ملی{' '}
                ...
                بر اساس روزنامه رسمى شماره {item.document_news_paper} که از این پس در این قرارداد &quot;ضامن حقوقی&quot; نامیده می‌شود
              </p>
            ))}
        </p>
